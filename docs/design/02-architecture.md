...
- **Linux-focused** — targets Linux (native, Flatpak, Snap) and Steam Deck.
...
| `open` | Open in file explorer (`xdg-open`). |
...
- **`discovery`** — Locate Steam installation root(s). Handle native, Flatpak, and Snap paths, plus custom library folders defined in `libraryfolders.vdf`.
...
- **`store`** — Read/write `cache.toml` from the cache directory (`~/.cache/steam-proton-browser/`).
...
| AUR / Cargo | Publish to package managers for easy install. |
...
- **Manual testing** on Linux and Steam Deck.
- **CI** via GitHub Actions (Linux).
